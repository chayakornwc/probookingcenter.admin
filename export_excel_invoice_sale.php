...
include_once('../unity/php_script.php');
    ...
    for($i = 0 ; $i< count($result);$i++){
                $period = $result[$i]['per_date_start'].'-'.d_m_y($result[$i]['per_date_end']);
        		...
        		$objPHPExcel->setActiveSheetIndex()->setCellValue('D'.$row,$result[$i]['qty']);
        		...
        		$objPHPExcel->setActiveSheetIndex()->setCellValue('G'.$row,$sale);
