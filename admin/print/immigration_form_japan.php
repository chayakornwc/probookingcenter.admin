...
// create new PDF document
        ...
        if($data_return['status'] == 200){
            ...
            # เพิ่มหน้า  
            $pageCount = $pdf->setSourceFile('template/japan/immigration_form_japan.pdf');
        ...
        foreach ($result as $key => $value) {
            ...
            # เขียนข้อมูล
                ...
                $pdf->SetXY(20,67.5);
                $pdf->Cell(45, 5, $falmily_name, $border, 1, 'L', 0, '', 1);

                $pdf->SetXY(82,67.5);
                $pdf->Cell(60, 5, $first_name, $border, 1, 'L', 0, '', 1);
                
                # กำหนด ฟอนท์  
		        $pdf->SetFont('angsanaupc', '', 16, '', true);  
                $pdf->SetXY(19.5,76);
                $pdf->Cell(8, 5, $day_of_birth, $border, 1, 'L', 0, '', 1);
                $pdf->SetXY(31.5,76);
                $pdf->Cell(8, 5, $month_of_birth, $border, 1, 'L', 0, '', 1);
                $pdf->SetXY(42.5,76);
                $pdf->Cell(10, 5, $year_of_birth, $border, 1, 'L', 0, '', 1);

                $pdf->SetFont('angsanaupc', '', 12, '', true);  
                $pdf->SetXY(80,76.5);
                $pdf->Cell(25, 5, $country, $border, 1, 'L', 0, '', 1);
                $pdf->SetXY(123,76.5);
                $pdf->Cell(25, 5, $city_state, $border, 1, 'L', 0, '', 1);

                $pdf->SetXY(120,82.5);
                $pdf->Cell(25, 5, $filght_no, $border, 1, 'L', 0, '', 1);

                $pdf->SetXY(120,90);
                $pdf->Cell(25, 5, $date_stay, $border, 1, 'L', 0, '', 1);
                

                $pdf->SetXY(20,99);
                $pdf->Cell(40, 5, $per_hotel, $border, 1, 'L', 0, '', 1);

              //  $pdf->SetFont('angsanaupc', '', 16, '', true);  
                $pdf->SetXY(112,99);
                $pdf->Cell(25, 5, '+'.$per_hotel_tel, $border, 1, 'L', 0, '', 1);
        ...
        # แสดงไฟล์ pdf  
        ob_start();
        $pdf->Output('tm_japan_'.$result[0]['ser_code'].'_'.date('d-m-Y_His',time()).'.pdf', 'I');  
        ob_end_flush();
