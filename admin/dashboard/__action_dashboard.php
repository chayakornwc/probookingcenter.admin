              ...
              $sql = "SELECT 
                ...
                if ($query->num_rows > 0) {
                    ...
                    while($row = $query->fetch_assoc()) {
                        ...
                        $_txt = '<a href="http://admin.probookingcenter.com/admin/booking/manage_booking.php?book_id='.$row['book_id'].'&book_code='.$row['book_code'].'" target="_blank" class="ui-status'.$cls.'">'.$row['sale_name'].' '.$row['qty'].' ('.  ucwords($row['company_name']). ')</a>';
