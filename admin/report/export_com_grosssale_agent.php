...
include_once('../unity/php_script.php');
	...
	$wsfile		= '/select_grosssale_agent.php'; //กำหนด File
	...
	if($data_return['status'] == 200){
      $result = $data_return['results'];
        ...
        $objPHPExcel = PHPExcel_IOFactory::load('template/report_grosssale_agent.xlsx');
    ...
    if ($_REQUEST['com_agency_id'] != ''){
        $com_name = 'บริษัท : '.$result[0]['agen_com_name'] ;
    }else {
       $com_name = 'บริษัท : ทั้งหมด';
    }
    if ($_REQUEST['agency_id'] != ''){
        $agentname = 'เอเจนซี่ : '.$result[0]['agentname'] ;
    }else {
       $agentname = 'เอเจนซี่ : ทั้งหมด';
    }
    ...
    $row_no = 8;
     for($i = 0 ; $i< count($result);$i++){
        if ($i == 0){
            $objPHPExcel->setActiveSheetIndex()->setCellValue('C4',$result[$i]['agen_com_name']);
            }else{
                if ($result[$i]['agen_com_name'] != $result[$i - 1]['agen_com_name']){
                  ...
                  //  $objPHPExcel->setActiveSheetIndex()->mergeCells('C'.$row.':E'.$row);
                    $objPHPExcel->setActiveSheetIndex()->setCellValue('C'.$row,$result[$i]['agen_com_name']);
                    $row++;
                    $no = 1;
                }
            }
        		...
        		$objPHPExcel->setActiveSheetIndex()->setCellValue('C'.$row,$result[$i]['agentname']);
    ...
    $strFileName = 'tmp/report_grosssale_agent_'.thai_date_short(strtotime($result[0]['per_date_start']))
