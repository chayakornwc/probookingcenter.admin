...
if($method != 'GET'){
    ...
    $sql = "SELECT  
                ...
                ,c.`air_name`
                ,d.`bus_qty` - (SELECT  COALESCE(SUM(e.`book_list_qty`),0) FROM `booking_list` e LEFT OUTER JOIN `booking` f on e.`book_code` = f.`book_code` 
                WHERE f.`per_id` =  a.`per_id` 
                AND e.`book_list_code` IN ('1','2','3','4','5')
                AND f.`bus_no` = d.`bus_no`)  AS 'qty_receipt'
                FROM `period` a
